...
<head>
    ...
    <title>Campus Points | TagisanAI</title>
...
<body class="bg-bglight dark:bg-bgdark min-h-screen w-full overflow-x-hidden font-poppins">
  ...
  <div class="min-h-screen pl-10 pr-10 md:pl-28 pt-12 md:pr-28">
    ...
    <div class="flex flex-col justify-center items-center md:justify-start md:items-start mb-8">
      <p class="text-xl md:text-4xl font-semibold text-dark dark:text-light">Campus Points</p>
      <p class="text-center md:text-start text-sm md:text-lg text-dark/60 dark:text-light/60 mt-2">
        Add and update points for each campuses based on events or performance.
      </p>
    </div>
          ...
          <table class="w-full border-collapse">
            ...
            <thead>
              <tr class="text-dark dark:text-light text-lg font-semibold">
                <th class="pl-6 pr-0 border-b bg-[hsl(23,100%,82%)] dark:bg-zinc-800/60 border-gray-500 dark:border-gray-600 text-left">Rank</th>
                <th class="px-6 py-3 border-b bg-[hsl(23,100%,82%)] dark:bg-zinc-800/60 border-gray-500 dark:border-gray-600 text-left">Campus</th>
            ...
            <tbody class="text-dark dark:text-light text-base">
              @foreach($localLeaderboard as $item)
              <tr class="hover:bg-[#FFE9C6]/70 dark:hover:bg-zinc-800/20 transition duration-200">
                <td class="pl-6 pr-0 py-4 font-bold border-b border-gray-400/70 dark:border-gray-600">{{ $item->rank }}</td>
                <td class="px-6 py-4 border-b border-gray-400/70 dark:border-gray-600">
                  <span class="font-bold">{{ $item->localCampuses->campus_code }}</span> {{ $item->localCampuses->campus_name }}
                </td>
