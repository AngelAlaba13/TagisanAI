...
<body class="bg-bglight dark:bg-bgdark min-h-screen w-full overflow-x-hidden font-poppins">
    ...
    <div class="min-h-screen pl-28 pt-12 pr-28">
        ...
        <div class=" bg-orange-200/40 dark:bg-base-200 flex justify-center items-center mt-20 mx-52 rounded-xl shadow-[0px_0px_10px_rgba(0,0,0,0.18)]">
            <!-- Main Form -->
          <form class="mt-10 space-y-10 w-full mx-10 p-5">

            <!-- Header Logo -->
            <div>
              <label class="flex justify-center items-center text-2xl font-semibold text-dark dark:text-light mb-2">Header Logo</label>
              <div>
                <label for="fileInput" id="dropZone"
                      class="w-full h-44 border-2 border-dashed border-black/60 dark:border-white/60 rounded-lg
                            flex flex-col items-center justify-center cursor-pointer hover:bg-primary/5 transition
                            text-dark dark:text-light bg-white/50 dark:bg-zinc-800">
                  <div id="uploadIcon" class="text-dark/80 dark:text-white/80 mb-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-11 h-11" fill="none" viewBox="0 0 24 24"
                        stroke="currentcolor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3"
                            d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M12 12V4m0 0l-3 3m3-3l3 3" />
                    </svg>
                  </div>
                  <p id="uploadText" class="text-dark dark:text-light font-medium">File Upload and Image Preview</p>
                </label>

                <input id="fileInput" type="file" name="file" accept="image/*" class="hidden" />

                <!-- Preview -->
                <div id="previewContainer" class="hidden mt-4 flex flex-col items-center">
                  <img id="previewImage" class="w-28 h-28 object-cover rounded-lg shadow-lg border border-base-300" />
                  <p id="fileName" class="mt-2 text-sm text-dark/70 dark:text-light/70"></p>
                </div>
              </div>
            </div>
            ...
            <!-- Event Name -->
            <div>
              <label class="block text-2xl font-semibold text-dark dark:text-light mb-2 ">Event Name</label>
              <input type="text" placeholder="Caravel 2025"
                    class="input input-bordered pl-2 w-1/2 text-dark dark:text-light bg-white dark:bg-zinc-800 border border-gray-400 dark:border-gray-500 shadow-sm" />
            </div>

            <!-- Color Scheme -->
            <div>
              <label class="block text-2xl font-semibold text-dark dark:text-light mb-4">Color Scheme</label>
              <div class="flex items-center gap-8">
                <div>
                  <p class="text-dark dark:text-light mb-2">Primary Color</p>
                  <input type="color" class="input w-24 h-12 p-0 bg-orange-200/40 dark:bg-base-200 cursor-pointer" value="#3b82f6" />
                </div>
                <div>
                  <p class="text-dark dark:text-light mb-2">Secondary Color</p>
                  <input type="color" class="input w-24 h-12 p-0 bg-orange-200/40 dark:bg-base-200 cursor-pointer" value="#60a5fa" />
                </div>
              </div>
            </div>
