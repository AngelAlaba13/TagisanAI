...
document.getElementById('ocrForm').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        const action = e.target.getAttribute('data-action') || e.target.action || '/localMasts/ocr/vision-extract';
  ...
  document.addEventListener('click', async (e) => {
    if (e.target && e.target.id === 'saveExtractedBtn') {
      ...
      try {
        const res = await fetch('/localM-events/bulk', {
          method: 'POST',
          headers: {
        ...
        setTimeout(() => {
            window.location.href = '/localMasts/Manage Events/manage-localM-events';
        }, 2000);
